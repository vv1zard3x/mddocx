...
Структура файла демонстрирует контракт `docxmd`:

- `#` (этот заголовок «Введение») — структурный элемент по ГОСТ 7.32,
  рендерится как `ВВЕДЕНИЕ` по центру, без номера.
- `##` ниже — содержательные разделы, нумеруются автоматически (`1`, `2`, ...).
- `###` — подразделы (`2.1`, `2.2`, ...).
- В конце документа — структурный элемент `Список использованных источников`
  (опять `#`, без номера).
...
1. [ГОСТ 7.32-2017](https://docs.cntd.ru/document/1200157208) — отчёт о
   научно-исследовательской работе, структура и правила оформления.
2. [ГОСТ 2.105-2019](https://docs.cntd.ru/document/1200166861) — ЕСКД,
